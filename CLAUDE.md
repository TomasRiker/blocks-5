...
- **A picture declares at its request whether it tiles** (`Texture::WM_WRAP`, `WM_REPEAT`),
  ...
  and `Texture::NEVER_PACK` beside the mode keeps one out for any other reason. **Nothing is
  clamped**: `GL_TEXTURE_WRAP_S` and `_T` are set nowhere in this game's history, so every gutter
  carries the picture's opposite edge, which is what GL's default returned.
...
| `rendering.md` | `renderer`, `renderstate`, `level`, `texture`, `textureatlas`, `tileset`, `sprite`, `engine`, `particlesystem`, `lava`, `lightning`, the crossfades | the renderer, its scopes and its bracket, what it bakes and why it is byte-exact, the files that own raw GL, browser colour, render layers, the FBO bind rule, the atlas and the two wrap modes |
