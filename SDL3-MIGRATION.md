...
**Read it with what has changed underneath since it was written.** ROADMAP item 54 put one renderer
under the whole game and took Emscripten's legacy GL emulation out of the browser build: there is no
`-sLEGACY_GL_EMULATION`, no `gl_immediate.cpp`, no fixed-function call and no GLU anywhere in the tree,
and the browser links Emscripten's plain WebGL library. Three parts of this plan rested on those, and each
says so where it stands: E-GL-WEB (§3) shrinks from a kill switch on the emulation to a probe of the
context SDL3 creates, Stage 2's third landmine (GLU vanishing from SDL3's Windows headers) has nothing
left to hit, and Stage 6's browser paragraph loses the `GLImmediate` initialisation it was built around.
`Tools/verify.py` also runs twenty-two checks today rather than the seventeen the check arithmetic in §4
starts from, and `syntax.sh` compiles 122 sources rather than 120: the increments stand, the bases do
not. Nothing else was re-surveyed, and the line numbers cited throughout are those of the tree the survey
read.
...
- The browser has been proven to reach **parity of failure** with SDL 1.2 in a test program, under the GL emulation that is gone now; nothing has been run against the renderer's own GL on a context SDL3 created (E-GL-WEB).
...
### E-GL-WEB — the context SDL3 creates, and whether the game's GL reaches it *(here, ~half a day)*

> The first edition made this the kill switch of the whole browser story, because the game then drew
> through `-sLEGACY_GL_EMULATION` and the survey had proved SDL3's context only to *parity of failure*
> with the emulation's `GLImmediate` initialisation. ROADMAP item 54 took the emulation out: the game
> draws through Emscripten's plain WebGL library, and what stood here — display lists, `glBegin`/`glEnd`
> quads, a texture-matrix scroll, the two-statement `Browser.useWebGL` fix — describes no code in the
> tree any more.

What remains is smaller and still **unproven**: nobody has run the game's GL against a context SDL3 created. A ~150-line Emscripten program linked against a scratchpad SDL3 that does what `Engine::init` and the renderer do: `SDL_CreateWindow` + `SDL_GL_CreateContext`, then the GL 2.0 entry points through `SDL_GL_GetProcAddress` (`glextensions.cpp`'s way), a program, a vertex buffer and an element buffer, an FBO with a packed depth-stencil renderbuffer, `glScissor`, a stencil pass, and `glReadPixels` off the FBO. Run in Chromium/swiftshader and read the frame back. The question it answers is whether the context SDL3's Emscripten backend makes current is the one the WebGL library's `GL.currentContext` serves, so that the entry points resolve and draw into the canvas the page shows — the whole of what the emulation's kill switch has become.

**Why it is still first:** everything in the browser plan is downstream of it and nothing has been spent yet.
...
#### `LinuxBuild/test/smoke.sh` — five new assertions, all green on SDL 1.2

> ROADMAP item 50 removed a sixth that stood here: `-nofbo` pinned the window to
> 640×480, and the assertion was to check that a maximize request did not change
> it. There is no `-nofbo`, no `Engine::fixWindowSize` and no
> `LinuxWindow::setFixedSize` any more, so nothing in this stage touches them.

1. **Fullscreen geometry with `B5_WM_PID` mandatory.** The assertion at `:213-236` exists but is inside `if [ -n "$B5_WM_PID" ]`; make its absence a failure, not a skip.
2. **`<Window fullscreen="1">` round-trips through `config.xml`** after Alt+Return. This is the one `<Window>` attribute testable here — `positionX`, `positionY` and `maximized` are all inside `#ifdef _WIN32`.
3. **The fourteen default binding ids, asserted as literal strings.** Not by round-trip: this box runs sdl12-compat, so a round trip records *its* mapping rather than real SDL 1.2's. `secondary="key:KP4"` and its three siblings must appear verbatim.
4. **Logic-tick rate.** Extend `Blocks5/src/testhooks.cpp`'s dump with `ticks` and `frames` counters and assert ticks/elapsed ≈ 50 Hz over a 10-second run. This is the assertion that later catches the accumulator bug — 15 ticks where 100 were wanted — and it must be passing *before* anything changes.
5. **Text input actually reaches an edit box.** Open the level editor's title field through `clickPath`, type a known string with `b5_key`, and assert the field's contents through the hook. Nothing in this tree has ever typed into an editor, and `SDL_StartTextInput` is the single easiest way to ship a build whose editors look fine and accept nothing.
...
*Say what the suite now costs.* The fullscreen `sleep 3`s, plus the new text-input, tick-rate (10 s) and F12 (2 s) steps, make a materially longer run than CLAUDE.md's *"they take about half a minute together"* describes. **Measure it in Stage 1 and write the new number into `LinuxBuild/README.md` and CLAUDE.md's "Checking a change" section**; the estimate to beat is two to three minutes for `smoke.sh` alone, and if it lands much past that, split the slowest steps into a second script the owner can run separately rather than let the main one rot from slowness.
...
**S3 — scale.** Not a screenshot question. Assert `computePresentRect`'s output through the test hook against the window size.
...
Then the part that makes any of it mean something: **break each guarded thing and watch the assertion fail.** Stub `LinuxWindow::setFullScreen` to `return false` → `smoke.sh` must fail. Overwrite a default binding id → the golden assertion must fail. Swap the channels in `img_save.cpp` → S1 must fail. Restore byte-for-byte. A smoke assertion proven only *green* is a grep that may never have matched — which is the very defect this stage is fixing.
...
**Landmine 3 — GLU on Windows — has nothing left to hit.** It was that SDL 1.2's `SDL_opengl.h` included `<GL/glu.h>` and SDL3's does not, so thirteen `glu*` call sites in six files would have stopped compiling on Windows alone, where the declarations arrived solely through SDL. ROADMAP item 54 deleted GLU from the tree — `Mat4` in `vec.h` builds `gluPerspective`'s and `gluLookAt`'s matrices as GLU built them, and `pch.h` and both links are without it — so there is no include to unguard and `sdl_contract.cpp` (below) carries no `glu*` call. `Tools/syntax.sh`, whose compiler and flags reproduced the break at the time, is what would notice a GLU call coming back.
...
Everything left in it is decided at compile time, so it needs no call site; the link-time half that stood here existed for the `glu*` calls, which went with GLU.
...
**Proven by.** All checks and all three harnesses, green and *identical* to Stage 1 — **except that `foreign_ifdef` now reports nothing**, which is this stage's headline. `verify.py` — **21 checks** (`tick_width` and `sdl_funnel` registered here), all clear. `selftest.py` — **25 cases, 25 fired.** `sh Tools/syntax.sh` is the specific proof of `sdl_contract.cpp`. A scratchpad harness that offsets the tick base past 2³² and asserts the REWIND blink state matches the 32-bit reference for all 1750 frames of the effect. Then the negative proof: revert each of the three landmine fixes in the scratchpad and confirm the corresponding check or compile fails.

**Ships?** Yes.

**Revert.** Five small independent commits; any one reverts alone.
...
**`WebBuild/build.sh`** — the **direct-compile route**. Replay `tools/ports/sdl3.py`'s own glob list against the vendored tree: exactly **181 files**, `emcc -c -O2 -sUSE_SDL=0`, 16.7 s on four cores, zero errors, `emar` → a 1,923,314 B archive, and a wasm **within 16 bytes** of the port's with identical browser output.
...
**`Tools/sdl3probe`** — ~60 lines each build system compiles and, on Linux and in the browser, runs: `SDL_Init`, `SDL_CreateWindow(OPENGL|RESIZABLE)`, `SDL_GL_CreateContext`, one quad through a program and a vertex buffer as the renderer draws, `glGetError`, report driver/version/stencil. This is the artefact that proves the three libraries *work* rather than merely link.
...
*Browser.* There is no emulation to bring up. The game draws through Emscripten's plain WebGL library, so after `SDL_GL_CreateContext` the one thing to establish is that the context SDL3 made current is the one `GL.currentContext` serves — E-GL-WEB's job, and the reason it runs before any of this. The two-statement `Browser.useWebGL` fix the first edition carried here belonged to `GLImmediate` and went with it.
...
**`SDL_SetWindowMinimumSize(640,480)` at creation**, so `handleResize`'s floor becomes book-keeping *because something else took over the enforcement*. The rest of what stood here — how to rebuild `fixWindowSize` out of `SDL_RestoreWindow`, `SDL_SetWindowMaximumSize` and `SDL_SetWindowResizable` — went with ROADMAP item 50: there is no fixed-size window to pin any more.
...
|---|---|---|---|---|
| 1 | `#ifdef SDL_VIDEO_DRIVER_X11` (`linux_window.cpp`) compiles to `return false` — Linux fullscreen (`engine.cpp:2491`) dies | No error, no warning, no link failure | `foreign_ifdef` **with `LinuxBuild` as a fifth base** (row 45); `#else #error`; smoke.sh fullscreen geometry made mandatory | 1, 2 |
...
| 4 | GLU vanishes on **Windows only** | — | **No longer reachable:** ROADMAP item 54 removed GLU from the tree, so there is no call site to lose | — |
...
| 21 | The GLImmediate fix written as one statement | — | **No longer reachable:** the GL emulation is gone (ROADMAP item 54); what E-GL-WEB still has to establish is the context, and smoke.js's frame-rendered assertion is what catches a context that draws nowhere | 0, 1 |
...
| 44 | `-Wl,--wrap=SDL_CreateRGBSurface` left pointing at a symbol that no longer exists | Named in the survey, tested never | Removed together with `platform_stubs.cpp` from `WebBuild/build.sh` | 6 |
...
10. **The window/placement passage** — `applyWindowStyle` and the `SDL_VIDEORESIZE` route both change mechanism; the placement block becomes portable.
...
14. **New:** who owns the canvas size in the browser, **and the payload cost** — about +700 KiB of wasm and +130 KiB of JS, roughly +65%, against a 49 KB drop in startup allocation.
...
1. **E-GL-WEB does not render a correct frame** — the renderer's program, buffers, FBO with packed depth-stencil, scissor and stencil pass, drawn on the context SDL3 created, in Chromium. Then nothing is spent: no vendoring, no git history, no restructure. This is deliberately the first and cheapest kill switch on the whole browser story, and a far smaller one than it was while the game ran on the GL emulation.
...
|---|---|---|
| 0 — Prove first (now including E-ENTRY-LOCAL) | 2–3 here | ~3 h Windows |
